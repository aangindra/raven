...
const { DEVICE_ID, PUSHER_APP_ID, PUSHER_APP_KEY, PUSHER_APP_SECRET } = process.env;
...
const sendMessage = async (cache, collection, config) => {
  ...
  try {
    ...
    if (foundMessage.type === "IMAGE" && foundMessage.image) {
      ...
      // await axios.post("https://app.whacenter.com/api/send", qs.stringify({
      //   device_id: DEVICE_ID,
