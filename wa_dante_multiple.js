...
const start = async () => {
  ...
  const worker = `saved_sessions/session-${WA_SESSION}`;
  ...
  const client = new Client({
    ...
    authStrategy: new LocalAuth({
      dataPath: path.join(__dirname, `saved_sessions/session-${WA_SESSION}`),
    }),
  });

  client.initialize();
  fs.rmdirSync(`${worker}/session/Default/Service Worker/`, { recursive: true });

  client.on('authenticated', (session) => {
    console.log('authenticated multi-device!')
  });
...
const sendMessage = async (client, cache, collection) => {
  ...
  try {
    ...
    const foo = await client.getFormattedNumber(foundMessage.phone)
    console.log({ foo })
